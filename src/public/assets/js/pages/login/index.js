...
async function login() {
    ...
    loginIsValid(email, password) ? await callAPI(email, password) ? window.location.href = `${Enums.Url}inicio` : showAlert('danger', 'Usuário ou senha incorretos!') : showAlert('warning', 'Preencha todos os campos!');
...
async function sessionDestroy() {
    ...
    .catch(function (jqXHR) {
        console.log('Falha ao destruir sessão!');
    })
}
